...
**酒馆是一个可以连接各种AI模型使用的网页平台。它提供了很多功能接口,可以直接看到/修改设置项。如果你知道github,那么可以继续了解到酒馆也是一个在gihub上开发的开源项目,而我们平时使用酒馆就是把这个项目下载/拉取到本地进行使用。**
...
- Top-p : 将令牌池限制为 K 个最有可能的令牌。数字越小,一致性越高,但创造性越低。将值设置为 1 可禁用其效果。举例说明:ai对某句话的回复可能性有五种,如果50%回复a,30%回复b,10%回复c,6回复d%,4%回复e,那么设置为96%,ai会在abcd中选一种而不会考虑e
